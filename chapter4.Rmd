...
**5.Divide the dataset to train and test sets, so that 80% of the data belongs to the train set.**
```{r echo = FALSE}
...
**6.Fit the linear discriminant analysis on the train set.**
...
**7.Save the crime categories from the test set and then remove the categorical crime variable from the test dataset, then predict the classes with the LDA model on the test data.**
...
**8.Reload the Boston dataset and standardize the dataset, and calculate the distances between the observations. **
```{r echo = FALSE}
data('Boston')
...
**9.Run k-means algorithm on the dataset.Investigate what is the optimal number of clusters and run the algorithm again. Visualize the clusters and interpret the results.**
```{r echo = FALSE}
# k-means clustering
...
```
The line chart shows that when the cluster is around 1.25, the total within sum of square is the highest.
